...
- Bound Apple Silicon MPS inference memory on unified RAM: free-memory estimates are capped by host available RAM, auto-batching also respects ``cpu_memory_budget_mb``, AUTO offload no longer treats a CPU copy as a second pool, host buffers are not pinned, ICL query-chunking is no longer CPU-only, and disk-backed stages release pages after each accelerator batch.
